...
const LoginForm = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      console.log("🔍 Login Response:", data); // Debugging purpose

      // ✅ Only proceed if backend returned success + token
      if (res.ok && data.token) {
        ...
        // ❌ Invalid login or backend error message
        alert(data.message || "❌ Invalid email or password!");
        localStorage.clear(); // Clear any wrong data
      }
    } catch (error) {
